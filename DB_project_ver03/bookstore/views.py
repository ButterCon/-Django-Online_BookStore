...
def orderConPage(request, name, book_id):
    #개인장바구니에 넣어서 구매하도록하자
    User_qs = User.objects.get(User_name=name)
    Card_qs = Card.objects.filter(User=User_qs).first() #카드 첫번째
    SD_qs = ShippingDestination.objects.get(User=User_qs)
    #장바구니에 책 넣기
    BookSB_qs = BookSB(Book=Book.object.get(id=book_id), ShoppingBasket=ShoppingBasket(User=User_qs))
    BookSB_qs.save()
    #쿠폰 할인 처리
    ...
    #Order 쿼리 저장
    BookSB_qs = BookSB.objects.filter(ShoppingBasket=ShoppingBasket.objects.get(User=User_qs))
    ...
    BookOrder_qs = BookOrder(Book=Book.object.get(id=book_id), Order=Order_qs)
    BookOrder_qs.save()
    BookOrder_qs = BookOrder.objects.filter(Order=Order.objects.get(User=User_qs))
    #리턴
    context = {'User_list': User_qs, 'SD_list': SD_qs, 'BookOrder_list': BookOrder_qs, 'BookSB_list': BookSB_qs}
    return render(request, 'bookstore/orderPage.html', context)

#수정해야됨
def orderPage(request, name, book_id):
    #개인장바구니에 넣어서 구매하도록하자
    User_qs = User.objects.get(User_name=name)
    Card_qs = Card.objects.filter(User=User_qs).first() #카드 첫번째
    SD_qs = ShippingDestination.objects.get(User=User_qs)
